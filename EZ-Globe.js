/**
 * GlobeLib-Three.js v4.1 (Robust Initializer)
 * This version uses an explicit init() function to receive user data.
 * This is the most robust and error-proof method, avoiding global scope issues.
 */
(function (window) {
  "use strict";

  // --- 1. PUBLIC API ---
  // The only function exposed to the user. It takes the configuration as an argument.
  function init(userConfig) {
    // A. Validate that the user passed the required data.
    if (!userConfig || !userConfig.customConfig || !userConfig.mySpotsData) {
      console.error(
        "GlobeLib Error: init() function requires an object with 'customConfig' and 'mySpotsData' properties."
      );
      document.body.innerHTML = `<div style="font-family:monospace;color:red;padding:2em;">GlobeLib Error: Configuration data not passed to init(). Please check the browser console.</div>`;
      return;
    }
    ...
    // C. Load dependencies and then initialize the scene.
    loadDependencies()
      .then(() => {
        console.log("GlobeLib: All dependencies loaded. Initializing scene.");
        const fullConfig = {
          ...userConfig.customConfig,
          spotsData: userConfig.mySpotsData
        };
        initializeScene(fullConfig);
      })
      .catch((error) => {
        console.error(
          "GlobeLib Error: Failed to load one or more required scripts.",
          error
        );
      });
  }
  ...
  function loadDependencies() {
    ...
    const promises = SCRIPT_URLS.map(
      (url) =>
  ...
  // --- 3. THE CORE GLOBE LOGIC (Unchanged) ---
  function initializeScene(config) {
    ...
    function updateInteractivity() {
      ...
      if (
        lastHovered &&
        !intersects.find((i) => i.object === lastHovered.core)
      ) {
      ...
      if (intersects.length > 0) {
        const hoveredBeacon = beacons.find(
          (b) => b.core === intersects[0].object
        );
        if (hoveredBeacon !== lastHovered) {
          gsap.to(hoveredBeacon.group.scale, {
            x: 1.5,
            y: 1.5,
            z: 1.5,
            duration: 0.3
    ...
    window.addEventListener("click", () => {
      if (lastHovered) {
        const data = lastHovered.core.userData;
        infoName.textContent = data.name;
        infoImage.src = data.image_url;
        infoDescEn.textContent = data.desc_en;
        infoDescZh.textContent = data.desc_zh;
        infoBox.style.display = "block";
        gsap.to(camera.position, {
          ...lastHovered.group.position.clone().multiplyScalar(2.0),
          duration: 1.5,
          ease: "power3.inOut"
        });
    ...
    function animate() {
      ...
      beacons.forEach((b) => {
        ...
        const pulseTime1 = elapsedTime * 1.5,
          pulseTime2 = elapsedTime * 1.5 + Math.PI;
        b.ring1.scale.setScalar(1 + (Math.sin(pulseTime1) + 1) / 2);
        b.ring1.material.opacity = 1 - (Math.sin(pulseTime1) + 1) / 2;
        b.ring2.scale.setScalar(1 + (Math.sin(pulseTime2) + 1) / 2);
        b.ring2.material.opacity = 1 - (Math.sin(pulseTime2) + 1) / 2;
      });
      updateInteractivity();
      controls.update();
  ...
  function injectLibraryAssets(config) {
    ...
    document.body.innerHTML = ""; // Clear the body before adding the container
    document.body.appendChild(container);
    const titleHtml = config.title
      ? `<div id="gl-title">${config.title}</div>`
      : "";
    const devHtml = config.developerName
      ? `<div id="gl-developer">By ${config.developerName}</div>`
      : "";
    container.insertAdjacentHTML(
      "beforeend",
      `<div id="gl-infoBox"><div id="gl-closeButton">×</div><h3 id="gl-infoName"></h3><img id="gl-infoImage" src="" alt="Location Image"><h4>Description</h4><p id="gl-infoDescEn"></p><h4>介绍</h4><p id="gl-infoDescZh"></p></div><div id="gl-resetButton">⟲</div>${titleHtml}${devHtml}`
    );
    return {
      container,

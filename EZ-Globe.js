...
(function(window) {
    ...
    function injectMetaTags() {
        ...
        if (!head.querySelector('meta[name="viewport"]')) { const viewport = document.createElement('meta'); viewport.setAttribute('name', 'viewport'); viewport.setAttribute('content', 'width=device-width, initial-scale=1.0'); head.prepend(viewport); }
    }
    function loadDependencies() {
        const SCRIPT_URLS = ["https://unpkg.com/three@0.128.0/build/three.min.js", "https://unpkg.com/three@0.128.0/examples/js/controls/OrbitControls.js", "https://cdnjs.cloudflare.com/ajax/libs/gsap/3.9.1/gsap.min.js"];
    ...
    function initializeScene(config) {
        ...
        controls.enableDamping = true; controls.dampingFactor = 0.05; controls.minDistance = 12; controls.maxDistance = 50; controls.enablePan = false;
        ...
        config.spotsData.forEach(spot => {
            ...
            core.userData = spot; beacon.add(core);
            const ring1 = new THREE.Mesh(new THREE.RingGeometry(0.15, 0.2, 32), new THREE.MeshBasicMaterial({ color: 0xffa500, side: THREE.DoubleSide, transparent: true })); beacon.add(ring1);
            const ring2 = new THREE.Mesh(new THREE.RingGeometry(0.15, 0.2, 32), new THREE.MeshBasicMaterial({ color: 0xffa500, side: THREE.DoubleSide, transparent: true })); beacon.add(ring2);
        ...
        function animate() {
            ...
            updateInteractivity(); controls.update(); renderer.render(scene, camera);
        }
        animate();
    }
